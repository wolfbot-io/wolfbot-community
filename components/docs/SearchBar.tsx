    ...
    <div className="relative w-full max-w-md">
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-2 bg-zinc-900 border border-zinc-800 rounded-xl text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/30 transition-colors"
        />
      </div>
        ...
        <div className="absolute top-full mt-2 w-full bg-zinc-900 border border-zinc-800 rounded-xl shadow-xl overflow-hidden z-50">
          {results.map((item) => (
            <Link
              key={item.slug}
              href={`/${item.slug}`}
              className="block px-4 py-3 hover:bg-zinc-800/50 transition-colors border-b border-zinc-800 last:border-0"
            >
      ...
      {open && query && results.length === 0 && (
        <div className="absolute top-full mt-2 w-full bg-zinc-900 border border-zinc-800 rounded-xl shadow-xl p-4 text-sm text-zinc-500 z-50">
