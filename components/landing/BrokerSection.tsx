...
export function BrokerSection({ copy, locale }: LocaleProps) {
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 border-t" style={{ borderColor: 'rgba(255,255,255,0.07)' }}>
      <h2 className="text-3xl font-bold text-white text-center mb-4">{c.brokersTitle || 'Crypto + Futures + MT5'}</h2>
      <p className="text-center mb-12 max-w-lg mx-auto" style={{ color: '#94A3B8' }}>
        {c.brokersSubtitle || 'One platform across modern digital markets and traditional financial markets.'}
      </p>
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 max-w-4xl mx-auto">
        {BROKERS.map((b) => (
          <Link
            key={b.slug}
            href={localizeHref(brokerHref(b), locale ?? null)}
      ...
      <div className="text-center mt-8">
        <Link href={localizeHref('/brokers', locale ?? null)} className="hover:underline text-sm" style={{ color: '#00C9E8' }}>{c.brokersViewAll || 'View all supported markets →'}</Link>
      </div>
    </section>
  )
}
