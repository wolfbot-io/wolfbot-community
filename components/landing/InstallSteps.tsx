  ...
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
      ...
      <div className="grid sm:grid-cols-3 gap-8 max-w-3xl mx-auto">
        {STEPS.map((step) => (
          <div key={step.number} className="text-center">
            <div className="w-12 h-12 rounded-full bg-blue-600/20 text-blue-400 flex items-center justify-center text-xl font-bold mx-auto mb-4">{step.number}</div>
      ...
      <div className="text-center mt-12">
        <Link href="/install/windows" className="text-blue-400 hover:underline text-sm">Windows guide →</Link>
        <span className="text-zinc-700 mx-3">·</span>
        <Link href="/install/linux" className="text-blue-400 hover:underline text-sm">Linux guide →</Link>
      </div>
    </section>
  )
}
